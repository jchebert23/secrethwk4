...
void addToHash(pair **table,int max, int p , int c){
    ...
    while(table[start]){start= (start + step) % max;}
...
pair * search(pair **table, int max, int p, int c){
    int start = init(p, c, max);
    while(table[start]){
    if(table[start]->pref == p && table[start]->nchar == c){ return table[start];}
